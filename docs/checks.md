...
| **Node 自检** | `npm run smoke` | 假 canvas 校验参数合法性 / 图元是否全被识别 / 绘制开销；直接构造 `App` 手动推帧，验证定格帧、固定步长、暂停、异常兜底、种子规则与逐字符守卫（13 收 / 6 拒）、"那个词不是种子、插播只多 2 发、不进分享链接也不进存储（存过的旧值会被抹掉）"，以及**「每日」开关**（第一次来自动打开、开着时换掉隔天的种子、运行中 `checkDay()` 跨日自己换场、暂停中不打扰、`?seed=` 与「随机」会关掉它）；并验证**调大预算真的会让画面变密**（450 → 1400 均值/峰值都要明显上升） |
| **真浏览器** | `npm run browser` | Playwright 驱动系统 Chromium，真开页面、真打字、真按键、真下载：① `file://` 打开构建产物断言"没报错 / 标题对 / 画面非空 / 种子一致 / 那个词写进 `?seed=` 不生效 / 老链接的 `?scene=` 被无视 / `?ui=0` 彻底无 UI"；② 开发页比对注入的 `<script>` 列表 == manifest、控制台便条只提字母个数不写出那个词、**逐字符真敲**那个词（敲完在飞 +2，且种子框刷回数字、`localStorage` 里只有数字、分享链接里只有数字、控制台不打印它）、快捷键（滑块聚焦时 R 要能放、种子框打字时 R 不放、回车收焦点、Esc 随时有效）；③ **录制**：面板收起、下载的 WebM 用 ffprobe 读到 `SEED`、浏览器能解码；④ **交互**：点画面落点精确、「每日」按钮亮起且 `fw.daily=1`、**带着昨天的种子打开不带 `?seed=` 的地址会自动换成今天这一场**、「随机」把开关关掉、剪贴板里的分享链接只有数字参数、PNG 文件名/元数据里只有数字种子、全屏生效 |
...
# 改物理：spoke 的 drag 5.0 → 5.2
 FAIL  seed=7            第 76 帧起行为变化 (预算 95 → 95，元素 69 → 69，线段 70 → 70)
（只有真的炸出 spoke 的那几个 seed 失败，其余照旧通过）

# 改渲染：圆头描边改成平头（物理与图元流完全没动）
 FAIL  seed=7            第 7 帧起行为变化     ← 老口径看不到这一类

# 去掉 scene、又把那个词从种子里摘出去那两次
  ~ seed=7 / 42 / 123456  指纹一个字节没变（只是基线的键从 random/seed=7 变成 seed=7）
  ~ classic/seed=7        移除                ← 那个"场"没了
  ~ seed=FIREWORKS        移除                ← 那个词不再是种子
  ~ seed=7+插播           新增                ← 插播片段还在，改成单独点名
```
...
| 换那个词 / 改种子规则 | `src/app.js` 的 `SECRET`、`seedContentOk()`；那个词触发的插播在 `src/show.js` 的 `interlude()`；哈希在 `src/rng.js` |
| 新编排 / 插播片段 | `src/show.js` |
| UI / 交互 | `index.html` + `src/ui.js`（全部 DOM，含逐字符守卫与控制台便条）；主循环在 `src/app.js`（不碰 DOM，可在 Node 里测） |
| 体量红线 | 每个文件 ≤ 300 行，`npm run check` 把关；目前最大 `src/app.js` 285 行 |
...
| `src/show.js` | 180 | 每帧顺序、绘制预算（容量）与节奏、那两发的插播、齐射、点击落点 |
| `src/view.js` | 119 | canvas 渲染器：仿射变换 + 整屏重绘 + 折线圆头描边 |
| `src/app.js` | 285 | 应用核心：状态、种子规则（只认数字）、「每日」开关与跨日换场、插播、固定步长主循环（**不碰 DOM**） |
| `src/ebml.js` | 181 | WebM 容器手术：把种子写进元数据（纯函数，Node 可测） |
| `src/record.js` | 102 | 录制机制：captureStream + MediaRecorder |
| `src/pngmeta.js` | 119 | PNG 容器手术：往 IHDR 后插 iTXt（种子 / 链接 / 时间），自带 CRC-32 |
| `src/actions.js` | 147 | 面板动作：存图 / 分享链接 / 全屏 / 录制 |
| `src/ui.js` | 213 | 界面本身：控制台、HUD、键盘、逐字符守卫、控制台便条、启动 |
