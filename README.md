...
1. **移植来源**：构图、配色、花型参数的出处（`reference/turtle_fireworks.py` 原样留档，
   不再有脚本加载它）。
2. **历史基线**：同一份原始设计在 git 历史里（`ba4e642`、`016eba9`），当年的逐帧/逐像素
   对拍脚本也在那里。以后若要分辨"这是我改的，还是当初移植就错了"，可以把那些工具取回来用。

**注意**：`src/rng.js` 仍与 CPython 的 `random.Random` 逐位一致 —— 这是**特性**不是兼容
包袱：`?seed=7` 这样的种子可以跨语言复现，`npm run rng` 负责守住它。
...
**每帧绘制预算**按实际开销算：折线按段数、其余各算 1，默认 450（原版口径时期是"图章数 130"，
同一个默认值在新的度量下会让画面明显变稀，所以重新标定过）。节流是启发式的（只影响后续发射的
火花数），所以单帧峰值会超过预算 —— 实测 400 帧内峰值 334~789。画不动就往下调滑块。
...
| UI / 交互 | `index.html` + `src/ui.js`（全部 DOM 在这） | 主循环逻辑改 `src/app.js`（不碰 DOM，可在 Node 里测） |
...
URL 参数：`?scene=classic|random|original&seed=7&frames=90&still=1&ui=0&max=450&w=924&h=691`

* `frames=N`：先走 N 个固定步再定格（第 1 帧 = `step(0)`，就是初始几何）
* `still=1`：定格在参考图那一帧（`?scene=original&still=1`）
* `ui=0` 隐藏全部 UI；`w` / `h` 钉死逻辑坐标系（构图与窗口大小无关）
...
| `src/rng.js` | 140 | MT19937 + CPython `init_by_array` 播种（seed 可跨语言复现） |
| `src/data.js` | 176 | 配色、圆点/线段/折线图元、花型参数表、截图实测数据 |
| `src/elements.js` | 192 | 图元基类、爆心闪光、余烬、火星（真实轨迹尾迹） |
| `src/trails.js` | 148 | 弹体、发射尾迹、钉死的折线 |
| `src/firework.js` | 141 | 发射 → 顶点炸开 |
| `src/show.js` | 199 | 每帧顺序、绘制预算与节流、三个场景、齐射 |
| `src/view.js` | 131 | canvas 渲染器：仿射变换 + 整屏重绘 + 折线圆头描边 |
| `src/app.js` | 188 | 应用核心：状态、固定步长主循环（**不碰 DOM**，可在 Node 里测） |
| `src/ui.js` | 131 | 全部 DOM：控制台、HUD、键盘、存 PNG、入口启动 |
...
docs/preview/         预览图（无头浏览器实拍，含尾迹口径对比）
```
