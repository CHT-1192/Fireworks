...
| 方式 | 怎么做 |
| --- | --- |
| **在线版** | 在仓库 Settings → Pages 选 **main 分支 / `/docs` 目录**，链接就是 `https://<用户名>.github.io/<仓库名>/`（`docs/index.html` 每次构建都会重写，不会过期） |
...
另外还有**一个词**能让开场换成**参考图风格**（左右两发照参考图的配色与花型）—— 控制台里留了
提示。照着往种子框里**一个字母一个字母**试：敲对了那个字符会留下来，敲错了会被弹回来。
...
- **录出来的 WebM 带着种子**：文件名和元数据里都写着 seed，拿到就能复现同一场
